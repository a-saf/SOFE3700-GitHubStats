...
<table style="width:50%" class="table" id="dataTable" >
    ...
    <tr style="font-weight:bold"><td>First Name</td><td>Last Name</td><td>Login Name</td></tr>
    <!--Using PHP to connect to database and fill up the table-->
    <?php

        //The Query Statement
        $sql = "SELECT u.fname, u.lname, u.login_name 
                FROM user as u
        ...
        while ($row = mysqli_fetch_assoc($result)){
            ...
            echo"<tr><td>{$row['fname']}</td><td>{$row['lname']}</td><td>{$row['login_name']}</td></tr>";
        }
    ?>  
</table>
<br> 

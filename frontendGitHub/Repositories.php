...
<br>
<div>
    <h1 class="hero-text">Repositories</h1>
</div>

<br><br>
<p> Description: Showing the Repositories table </p>
<br>
...
<table style="width:100%" class="table" id="dataTable" >
    ...
    <tr style="font-weight:bold"><td>Repository ID</td><td>repo_size</td><td>repo_name</td><td>repo_owner</td><td>no_of_forks</td><td>Languages</td><td>Date Created</td><td>Date Updated</td></tr>
    ...
    <?php
        ...
        while ($row = mysqli_fetch_assoc($result)){
            ...
            echo "<tr><td>{$row['repositoryID']}</td><td>{$row['repo_size']}</td><td>{$row['repo_name']}</td><td>{$row['repo_owner']}</td><td>{$row['no_of_forks']}</td><td>{$row['languages']}</td><td>{$row['date_created']}</td><td>{$row['date_updated']}</td></tr>";

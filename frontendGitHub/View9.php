...
<br>
<div>
    <h1 class="hero-text">VIEW 9</h1>
</div>

<br><br>
<p> Description: Show all issues for repository ID 412306 </p>
<br>
...
<table style="width:50%" class="table" id="dataTable" >
    ...
    <tr style="font-weight:bold"><td>Repo ID</td><td>Issues ID</td><td>Issue_num</td><td>Issue_state</td><td>Created_at</td><td>closed_at</td></tr>
    ...
    <?php
        ...
        while ($row = mysqli_fetch_assoc($result)){
            ...
            echo "<tr><td>{$row['repo_id']}</td><td>{$row['issue_id']}</td><td>{$row['issue_num']}</td><td>{$row['issue_state']}</td><td>{$row['created_at']}</td><td>{$row['closed_at']}</td></tr>";
        }
    ?>  
</table>
<br> 

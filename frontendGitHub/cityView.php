...
<br>
<div>
    <h1 class="hero-text">CITY VIEW</h1>
</div>
...
<table style="width:50%" class="table" id="dataTable" >
    ...
    <tr style="font-weight:bold"><td>City</td><td>total</td></tr>
    <!--Using PHP to connect to database and fill up the table-->
    <?php

        //The Query Statement
        $sql = "SELECT city, FORMAT(count(city), 'D3') AS total 
                FROM user 
                GROUP by city;";
...
<table style="width:100%">
    ...
    <tr style="font-weight:bold"><td>First Name</td><td>Last Name</td><td>Login Name</td><td>City</td></tr>

    <?php

        $sql = "SELECT fname, lname, login_name, city FROM user";
        $result = mysqli_query($con, $sql) or die("Bad Query: $sql");

        while ($row = mysqli_fetch_assoc($result)){

            echo"<tr><td>{$row['fname']}</td><td>{$row['lname']}</td><td>{$row['login_name']}</td><td>{$row['city']}</td></tr>";
        }	
    ?>  
</table>
<br>

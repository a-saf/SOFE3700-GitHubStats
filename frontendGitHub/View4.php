...
<br>
<div>
    <h1 class="hero-text">VIEW 4</h1>
</div>

<br><br>
<p> Description: Full Join of User and Repository Table
<br>
...
<div class = "table_wrapper">
    ...
    <table style="width:100%" class="table" id="dataTable" >
        ...
        <tr style="font-weight:bold"><td>User ID</td><td>Login Name</td><td>First Name</td><td>Last Name</td><td>Repository URL</td><td>Company</td><td>City</td><td>no_of_repos</td><td>Followers</td><td>Followings</td><td>Date Created</td><td>Date Updated</td><td>Repository ID</td><td>repo_size</td><td>repo_name</td><td>repo_owner</td><td>no_of_forks</td><td>Languages</td><td>Date Created</td><td>Date Updated</td></tr>
        ...
        <?php
            ...
            while ($row = mysqli_fetch_assoc($result)){
                ...
                echo "<tr><td>{$row['user_id']}</td><td>{$row['login_name']}</td><td>{$row['fname']}</td><td>{$row['lname']}</td><td>{$row['repo_url']}</td><td>{$row['company']}</td><td>{$row['city']}</td><td>{$row['no_of_repos']}</td><td>{$row['followers']}</td><td>{$row['followings']}</td><td>{$row['date_created']}</td><td>{$row['date_updated']}</td><td>{$row['repositoryID']}</td><td>{$row['repo_size']}</td><td>{$row['repo_name']}</td><td>{$row['repo_owner']}</td><td>{$row['no_of_forks']}</td><td>{$row['languages']}</td><td>{$row['date_created']}</td><td>{$row['date_updated']}</td></tr>";
            }
        ?>  
    </table>
</div>
